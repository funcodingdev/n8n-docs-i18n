---
title: Postgres Chat Memory node 文档
description: >-
  了解如何在 n8n 中使用 Postgres Chat Memory node。按照技术文档将 Postgres Chat
...
使用 Postgres Chat Memory node 将 Postgres 用作存储 chat history 的 memory[^1] server。

在本页中，你可以找到 Postgres Chat Memory node 支持的 operations 列表，以及更多相关资源链接。
...
* **Table Name**：输入用于存储 chat history 的 table 名称。如果 table 不存在，系统会创建它。
* **Context Window Length**：输入要作为 context 考虑的前序 interactions 数量。

## 相关资源 <a href="#related-resources" id="related-resources"></a>

有关该服务的更多信息，请参阅 [LangChain Postgres Chat Message History 文档](https://js.langchain.com/docs/integrations/memory/postgres)。

{% include "https://app.gitbook.com/s/GixZThfitWP21x2gQFpD/~/reusable/mjXhKRIw98UJ5hk9LWBl/" %}

## 单一 memory instance <a href="#single-memory-instance" id="single-memory-instance"></a>

如果你向 workflow 添加多个 Postgres Chat Memory node，默认情况下所有 nodes 都会访问同一个 memory instance。在执行会覆盖现有 memory contents 的破坏性 actions 时请谨慎，例如 [Chat Memory Manager](./n8n-nodes-langchain.memorymanager.md) node 中的 override all messages operation。如果想在 workflow 中使用多个 memory instance，请在不同 memory nodes 中设置不同 session IDs。
