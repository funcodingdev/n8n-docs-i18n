...
description: >-
  了解如何在 n8n 中使用 QuestDB node。按照技术文档将 QuestDB node 集成到你的 workflow 中。
contentType:
  - integration
  - reference
...
使用 QuestDB node 自动化 QuestDB 中的工作，并将 QuestDB 与其他应用集成。n8n 支持通过 QuestDB 执行 SQL query，并向数据库中插入 row。

在本页中，你可以找到 QuestDB node 支持的操作列表，以及更多资源链接。

{% hint style="info" %}
**Credentials**

有关设置身份验证的指导，请参阅 [QuestDB credentials](../credentials/questdb.md)。
{% endhint %}
...
[浏览 QuestDB node 文档集成模板](https://n8n.io/integrations/questdb)或[搜索所有模板](https://n8n.io/workflows/)
